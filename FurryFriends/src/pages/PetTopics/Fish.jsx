...
const FishPosts = ({ toggleTheme, darkMode }) => {
  ...
  // Fetch posts tagged "Fish" and sort by likes descending
  useEffect(() => {
    const fetchFishPosts = async () => {
      try {
        const response = await api.get("/posts/tag/Fish");
        ...
        console.error("Error fetching fish posts:", error);
      }
    };
    fetchFishPosts();
  }, []);

  // Retrieve current user ID (assuming stored in localStorage)
