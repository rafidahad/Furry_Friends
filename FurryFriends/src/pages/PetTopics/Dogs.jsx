...
const DogsPosts = ({ toggleTheme, darkMode }) => {
  ...
  // Fetch posts tagged "Dogs" and sort by likes descending
  useEffect(() => {
    const fetchDogsPosts = async () => {
      try {
        // Here we assume you have an endpoint that returns posts with the petTag "Dogs"
        const response = await api.get("/posts/tag/Dogs");
        ...
        console.error("Error fetching dogs posts:", error);
      }
    };
    fetchDogsPosts();
  }, []);

  // Get current user id (assuming it's stored in localStorage as part of the user object)
