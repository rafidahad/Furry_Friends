...
const GuineaPigsPosts = ({ toggleTheme, darkMode }) => {
  const [posts, setPosts] = useState([]);
  const [mobileOpen, setMobileOpen] = useState(false);
  // State for comment UI per post
  ...
  // Fetch posts tagged "Guinea Pigs" and sort by descending likes
  useEffect(() => {
    const fetchGuineaPigsPosts = async () => {
      try {
        // Make sure the endpoint matches your backend's expected tag format.
        const response = await api.get("/posts/tag/Guinea Pigs");
        ...
        console.error("Error fetching guinea pigs posts:", error);
      }
    };
    fetchGuineaPigsPosts();
  }, []);

  // Retrieve current user id (assuming it's stored in localStorage as part of the user object)
  const currentUser = JSON.parse(localStorage.getItem("user"));
  const currentUserId = currentUser?._id || localStorage.getItem("userId");

  // Like functionality: call API and update local state
...
export default GuineaPigsPosts;
